...
def get_data():
    ...
    try:
        ...
        # Hata yakalayıcı blok (Bu kısım önceki kodda eksikti)
...
if menu == "🎓 DEMOGRAFİK İSTİHBARAT" and user['Rol'] == 'ADMIN':
    ...
    tab1, tab2, tab3 = st.tabs(["🏛️ ÜNİVERSİTE ANALİZİ", "🌍 BÖLGESEL İSTİHBARAT", "🏢 KURUMSAL İSTİHBARAT"])
    ...
    with tab2:
        ...
        if target_region:
            ...
            with st.expander(f"📋 {target_region} Üye Listesi ({len(df_reg)} Kişi)"):
                st.dataframe(df_reg[['Sicil_No', 'Ad_Soyad', 'Universite', 'Yas', 'Kurum', 'Telefon']], use_container_width=True)
    ...
    with tab3:
        ...
        with c_k2:
            ...
            if not df_kurum.empty:
                fig_ka = px.pie(df_kurum, names='Yas_Grubu', hole=0.5)
